...
class CartesianFrame extends JFrame
{
	...
	public JButton restart=new JButton("Close");
	...
	// x-axis
     ...
     Y_AXIS_X_COORD, Y_AXIS_FIRST_Y_COORD);
      ...
      / yCoordNumbers;
		...
		for(int j=0;j<Dashboard.criticalName.size();j++)
		{	
			
			if(j==0)
			{
				g2.drawString(Dashboard.criticalName.get(j)+" - "+Dashboard.cridur.get(j),
				...
				g2.drawLine(X_AXIS_FIRST_X_COORD, yConstant,
						X_AXIS_FIRST_X_COORD + (Dashboard.criEF.get(j)* xLength),yConstant);
			...
			else
			{
				...
				g2.setColor(color);
				g2.drawString(Dashboard.criticalName.get(j)+" - "+Dashboard.cridur.get(j),
						X_AXIS_FIRST_X_COORD +(Dashboard.criES.get(j)*xLength)+20, yConstant+increase);
				//g2.drawString(alert.criticalName.get(j)+" , ",160+increase,600);
				g2.setColor(cc);
				g2.drawLine(X_AXIS_FIRST_X_COORD +(Dashboard.criES.get(j)*xLength), yConstant+increase, 
						X_AXIS_FIRST_X_COORD +(Dashboard.criEF.get(j)*xLength), yConstant+increase);
		...
		for(int j=0;j<Dashboard.noncriticalName.size();j++)
		{
			increase+=20;
			g2.setColor(color);
			g2.drawString(Dashboard.noncriticalName.get(j)+" - "+Dashboard.nondur.get(j),
					X_AXIS_FIRST_X_COORD +(Dashboard.nonES.get(j)*xLength)+20, yConstant+increase);
			g2.drawLine(X_AXIS_FIRST_X_COORD +(Dashboard.nonES.get(j)*xLength), yConstant+increase, 
						X_AXIS_FIRST_X_COORD +(Dashboard.nonLF.get(j)*xLength), yConstant+increase);
